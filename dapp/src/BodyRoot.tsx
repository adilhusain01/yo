...
function BodyRoot(props: BodyRootProps) {
  ...
  useEffect(() => {
    // try to set config address from environment variable first, then fallback to other methods
    console.log("useEffect triggered for wrapper:", wrapper);
    if (wrappers && wrappersConfig) {
      // First try environment variable
      const envAddress = import.meta.env.VITE_CONTRACT_ADDRESS;
      console.log("Environment address:", envAddress);
      ...
      // Then try default address from config (only if not empty)
      ...
      try {
        if (props.addressFromUrl && props.addressFromUrl.trim() !== "") {
          setConfigAddress(Address.parse(props.addressFromUrl));
          return;
        }
      } catch (e) {
        console.error("Failed to parse address from URL:", e);
      }
      ...
      try {
        ...
        console.error("Failed to parse provided address:", e);
      }

      // If all fail, set to null to show input
  ...
  const buildAndExecute = async (isGet: boolean, methodName: string, params: ParamsWithValue) => {
    ...
    // Check if we have either a valid manual address or a config address
    if (!configAddress && (!destAddr || addressError)) {
                      ...
                      return (
                        <MenuItem
                          ...
                          onClick={() => {
                            onClose();
                            setMethod(methodName);
                            // Don't reset configAddress - keep the environment variable
                            setActionCardKey(methodName);
                            setTimeout(() => onOpen(), 100);
                          }}
                        >
